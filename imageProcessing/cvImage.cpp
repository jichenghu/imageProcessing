...
void CcvImage::extractFeatures ( Ptr<FeatureDetector> detector, Ptr<DescriptorExtractor> extractor, vector<KeyPoint>& keyPoints, Mat& imgDesc )
{
	Mat	imgGray;

	cvtColor ( m_img, imgGray, CV_RGB2GRAY );

	detector->detect ( imgGray, keyPoints );
	extractor->compute ( imgGray, keyPoints, imgDesc );
}

void CcvImage::extractFeatures ( Ptr<FeatureDetector> detector, Ptr<DescriptorExtractor> extractor )
{
	extractFeatures ( detector, extractor, m_keyPoints, m_imgDesc );
}
